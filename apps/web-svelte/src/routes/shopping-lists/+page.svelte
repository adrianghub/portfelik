<script lang="ts">
	...
	import { fetchShoppingLists, createShoppingList, deleteShoppingList } from '$lib/services/shopping-lists';
...
<div class="container mx-auto max-w-3xl px-4 py-6 space-y-5">
	<div class="flex items-center justify-between">
		...
		<button
			onclick={() => { showCreate = true; newName = ''; newGroupId = ''; }}
...
<!-- Create dialog -->
<Dialog open={showCreate} onclose={() => (showCreate = false)} title={m.shopping_list_form_title_add()}>
	<form onsubmit={submitCreate} class="space-y-4">
		<div class="space-y-1">
			<label class="text-xs font-medium text-zinc-600" for="sl-name">{m.shopping_list_form_name()}</label>
			<input
				id="sl-name"
				type="text"
				required
		...
		{#if groupsQuery.data && groupsQuery.data.length > 0}
			<div class="space-y-1">
				<label class="text-xs font-medium text-zinc-600" for="sl-group">{m.shopping_list_form_group()}</label>
				<select
					id="sl-group"
		...
		<div class="flex gap-2 pt-1">
			<button type="button" onclick={() => (showCreate = false)} class="flex-1 rounded-lg border border-zinc-200 py-2 text-sm font-medium text-zinc-600 hover:bg-zinc-50 transition-colors">
				{m.common_cancel()}
			</button>
			<button type="submit" disabled={createMut.isPending} class="flex-1 rounded-lg bg-zinc-900 py-2 text-sm font-medium text-white hover:bg-zinc-700 disabled:opacity-50 transition-colors">
