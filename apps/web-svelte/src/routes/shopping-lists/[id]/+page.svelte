...
<div class="container mx-auto max-w-2xl space-y-4 px-4 py-6">
  <a
    href="/shopping-lists"
    class="inline-flex items-center gap-1 text-sm text-zinc-500 transition-colors hover:text-zinc-900"
  >
    <svg
  ...
  {#if query.isLoading}
    <div class="space-y-3">
      <div class="h-8 w-48 animate-pulse rounded-lg bg-zinc-100"></div>
      <div class="space-y-2">
        {#each [0, 1, 2, 3, 4] as _, i (i)}
          <div class="h-12 animate-pulse rounded-xl bg-zinc-100"></div>
        {/each}
      </div>
    </div>
  ...
  {:else if query.data}
    ...
    {#if list.shopping_list_items.length === 0}
      <p class="py-8 text-center text-sm text-zinc-400">{m.shopping_list_items_empty()}</p>
    {:else}
      <ul class="space-y-1">
          ...
          <li class="flex items-center gap-3 rounded-xl border border-zinc-100 bg-white px-4 py-3">
            ...
            {:else}
              <div
                class={cn(
                  ...
                  item.completed ? "border-zinc-800 bg-zinc-800" : "border-zinc-300"
                )}
              >
                {#if item.completed}
                  <svg
            ...
            {#if isActive}
              <button
                ...
                class="shrink-0 p-1 text-zinc-300 transition-colors hover:text-rose-500"
    ...
    {#if isActive}
      <div class="flex gap-2">
        <button
          ...
          class="flex-1 rounded-xl border border-dashed border-zinc-300 py-2.5 text-sm font-medium text-zinc-500 transition-colors hover:border-zinc-400 hover:text-zinc-700"
        >
    ...
    {#if list.total_amount != null}
      <div
        class="flex items-center justify-between rounded-xl border border-zinc-200 bg-white px-4 py-3"
      >
  ...
  <form onsubmit={submitAddItem} class="space-y-4">
    ...
    <div class="flex gap-3">
      <div class="flex-1 space-y-1">
        <label class="text-xs font-medium text-zinc-600" for="item-qty"
      ...
      <div class="flex-1 space-y-1">
        <label class="text-xs font-medium text-zinc-600" for="item-unit"
...
>
  <form onsubmit={submitComplete} class="space-y-4">
    <div class="space-y-1">
      <label class="text-xs font-medium text-zinc-600" for="comp-amount"
    ...
    <div class="space-y-1">
      <label class="text-xs font-medium text-zinc-600" for="comp-cat"
