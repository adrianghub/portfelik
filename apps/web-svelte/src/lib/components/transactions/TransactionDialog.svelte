<script lang="ts">
	...
	let date = $state(untrack(() => (initial?.date ? initial.date.slice(0, 10) : new Date().toISOString().slice(0, 10))));
...
<Dialog {open} {onclose} {title}>
	<form onsubmit={handleSubmit} class="space-y-4">
		<!-- Type toggle -->
		<div class="flex rounded-lg border border-zinc-200 overflow-hidden text-sm">
			<button
				type="button"
				onclick={() => (type = 'expense')}
				class="flex-1 py-2 font-medium transition-colors {type === 'expense' ? 'bg-rose-600 text-white' : 'bg-white text-zinc-500 hover:bg-zinc-50'}"
			>
				{m.common_expense()}
			</button>
			<button
				type="button"
				onclick={() => (type = 'income')}
				class="flex-1 py-2 font-medium transition-colors {type === 'income' ? 'bg-emerald-600 text-white' : 'bg-white text-zinc-500 hover:bg-zinc-50'}"
			>
				{m.common_income()}
			</button>
		</div>
		...
		<!-- Amount -->
		<div class="space-y-1">
			<label class="text-xs font-medium text-zinc-600" for="tx-amount">{m.transaction_form_amount()}</label>
			<input
				id="tx-amount"
				type="number"
				min="0.01"
		...
		<!-- Description -->
		<div class="space-y-1">
			<label class="text-xs font-medium text-zinc-600" for="tx-desc">{m.transaction_form_description()}</label>
			<input
				id="tx-desc"
				type="text"
				required
		...
		<!-- Date -->
		<div class="space-y-1">
			<label class="text-xs font-medium text-zinc-600" for="tx-date">{m.transaction_form_date()}</label>
			<input
				id="tx-date"
				type="date"
				required
		...
		<!-- Category -->
		<div class="space-y-1">
			<label class="text-xs font-medium text-zinc-600" for="tx-cat">{m.transaction_form_category()}</label>
			<select
				id="tx-cat"
				required
		...
		<!-- Status -->
		<div class="space-y-1">
			<label class="text-xs font-medium text-zinc-600" for="tx-status">{m.transaction_form_status()}</label>
			<select
				id="tx-status"
				bind:value={status}
				class="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900/10 bg-white"
		...
		<label class="flex items-center gap-3 cursor-pointer select-none">
			...
			<div class="relative h-5 w-9 rounded-full transition-colors {is_recurring ? 'bg-zinc-800' : 'bg-zinc-200'}">
				<div class="absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform {is_recurring ? 'translate-x-4' : 'translate-x-0'}"></div>
			</div>
		...
		{#if is_recurring}
			<div class="space-y-1">
				<label class="text-xs font-medium text-zinc-600" for="tx-recday">{m.transaction_form_recurring_day()}</label>
				<input
					id="tx-recday"
					type="number"
					min="1"
