      ...
      <div class="mt-3">
        ...
        <div class="mt-1.5 flex items-center justify-between gap-2 text-xs">
          <span class="text-slate-400">
            wydano <span class="font-semibold text-slate-200"
              >{formatCurrency(list.linkedAmount)}</span
            >
            z {formatCurrency(list.total_amount!)}
          </span>
          <span
            class={cn(
    ...
    {#if eligibleCount > 0 && isActive}
      <div class="mt-2.5 flex items-center justify-between gap-2">
        ...
        >
          ...
          {m.plan_card_suggestions({ count: eligibleCount })}
        </span>
