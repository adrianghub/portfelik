...
<div class="relative" data-notif-popover>
  ...
  >
    ...
    {#if unreadCount > 0}
      <span
        class="absolute -top-0.5 -right-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-zinc-900 text-[9px] font-bold text-white"
      >
  ...
  {#if open}
    ...
    >
      ...
      <ul class="max-h-80 divide-y divide-zinc-50 overflow-y-auto">
        {#if query.isLoading}
          {#each [0, 1, 2] as _, i (i)}
            <li class="px-4 py-3">
              <div class="h-3 w-3/4 animate-pulse rounded bg-zinc-100"></div>
              <div class="mt-1.5 h-2 w-1/2 animate-pulse rounded bg-zinc-100"></div>
            </li>
          {/each}
        ...
        {:else}
          {#each notifications as n (n.id)}
            ...
            <li
              class="group relative flex gap-3 px-4 py-3 transition-colors hover:bg-zinc-50"
              class:bg-blue-50={isUnread}
            >
              {#if isUnread}
                <div class="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-zinc-900"></div>
              {:else}
              ...
              <div class="min-w-0 flex-1">
                <button
                  ...
                  disabled={!isUnread}
                >
                  <p class="truncate text-xs font-medium text-zinc-900">{n.title}</p>
                  <p class="mt-0.5 line-clamp-2 text-xs text-zinc-500">{n.body}</p>
                </button>
                <span class="mt-1 block text-[10px] text-zinc-400">
              ...
              <button
                ...
                class="absolute top-3 right-3 hidden p-0.5 text-zinc-300 transition-colors group-hover:block hover:text-zinc-600"
                aria-label="Usuń"
              >
                <X size={12} />
              </button>
