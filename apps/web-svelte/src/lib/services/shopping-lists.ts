...
import type { ShoppingList, ShoppingListItem, ShoppingListWithItems, Transaction } from '$lib/types';
...
export async function updateShoppingListItem(
	id: string,
	updates: Partial<{ name: string; completed: boolean; quantity: number | null; unit: string | null; position: number }>
): Promise<ShoppingListItem> {
	const { data, error } = await supabase
		.from('shopping_list_items')
		.update(updates)
