...
**Immediate next step:** Shopping-list enhancements were pushed to `dev` on 2026-05-24 — item category sections + editable vocabulary are implemented, local/staging persona seed covers default shopping item categories plus manual `admin@portfelik.test` and `user@portfelik.test` accounts (password = login), and the P2 badge review fixes are local: `/transactions?txId=...` now fetches linked transactions outside the active date range, and shopping-item Undo only appears after confirmed delete. The dashboard AI/observability panel ("Sygnały z okresu") is parked locally until the product is more stable. CI hardening is local: upstream `ci` now enforces `format:check`, isolated `test:unit`, and blocking full-history Gitleaks before RLS/e2e/deploy, with a PR template and unit coverage for dashboard daily rotation + utils helpers. Current gates: svelte-check 0/0, lint, format, unit 16/16, RLS list unchanged, focused transactions + shopping-list Playwright 23/23, build, diff check, full-history gitleaks, and changed-file secret scan reviewed. Next: push the local review-fix/UI cleanup + CI-hardening commits, then let staging apply `20260528000000_shopping_list_items_category.sql` and `pnpm seed:staging` before browser/smoke verification. Going forward, never apply prod/staging schema out-of-band (MCP `apply_migration` / linked query) — let merges to the synced branch be the only path that mutates remote schema, or history drifts again.
...
| Dashboard AI observability module — period-aware finance signals: transaction count, income/expense split, top expense category, and one actionable mission                                                                                                                                                                                                                                                                                                                                                                     | ⏸️ Parked locally 2026-05-24 — "Sygnały z okresu" panel removed from dashboard until the core product is stable; svelte-check 0/0, lint clean, format clean                               |
...
| Unit combobox presets + custom entry                                                                                                       | ✅ Done locally 2026-05-23 — shopping-list unit field now defaults to szt. and offers kg/opak./l presets with custom input                                                                                                                                          |
| Item category sections + editable vocabulary                                                                                               | ✅ Local 2026-05-24 — `shopping_list_items.category`, owner-scoped `shopping_item_categories`, category-first section workflow, grouped/collapsible render, settings vocabulary                                                                                     |
| 7 — Mocked Playwright stabilization bundle                                                                                                 | ✅ Local on `dev` 2026-05-24 — focused stabilization suite 16/16 green after category-section closeout                                                                                                                                                              |
...
| 10 — Full gates, docs, staging verification                                                                                                | ✅ Local closeout done 2026-05-24 — Paraglide, svelte-check 0/0, lint, format, Vitest 153/153, focused shopping-list Playwright 16/16, local `pnpm seed:local` verified, secret scan only matched CLAUDE.md rule text; staging verification remains next after push |
| 11 — Shopping-list detail UI closeout                                                                                                      | ✅ Local 2026-05-24 — empty detail state, unchecked-item completion gate, category sections, completed-card tx chip, mobile fixed CTA; focused shopping-list Playwright 16/16                                                                                       |
| 12 — P2 badge review fixes                                                                                                                 | ✅ Local 2026-05-24 — linked transaction badge deep links fetch by ID outside active date filters; shopping-item delete Undo is emitted only after confirmed delete; focused Playwright 23/23                                                                       |
...
| Production migration-history normalization                                                                                                 | ✅ Done 2026-05-24 — rewrote prod `schema_migrations` to mirror `main`'s 32 files via MCP (all DDL pre-verified applied; backup `schema_migrations_backup_20260524`). Fixes GitHub-integration "Supabase Preview" check                                             |
