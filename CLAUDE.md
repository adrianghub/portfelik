...
**Immediate next step:** Production migration-history normalization is DONE (2026-05-24) — prod `supabase_migrations.schema_migrations` now exactly mirrors `main`'s 32 migration files (versions+names), unblocking the Supabase GitHub-integration "Supabase Preview" check. Re-run that check on `main` to confirm 0 pending. Next: enable the GitHub integration on `portfelik-staging` (production branch = `dev`), then finish the first dedicated `dev.portfelik.pages.dev` deploy/smoke verification before bank-import Step 6 or mortgage/debt planning. Going forward, never apply prod/staging schema out-of-band (MCP `apply_migration` / linked query) — let merges to the synced branch be the only path that mutates remote schema, or history drifts again.
...
| Item category sections + editable vocabulary                                                                                               | ✅ Local 2026-05-23 — `shopping_list_items.category`, owner-scoped `shopping_item_categories`, category-first section workflow, grouped/collapsible render |
| 7 — Mocked Playwright stabilization bundle                                                                                                 | ✅ Local on `dev` 2026-05-22 — 10 stabilization regression cases; closeout coverage adds completed-card chip + mobile detail CTA                           |
...
| 11 — Shopping-list detail UI closeout                                                                                                      | ✅ Local 2026-05-22 — empty detail state, unchecked-item completion gate, completed-card tx chip, mobile fixed CTA; focused shopping-list Playwright 17/17 |
