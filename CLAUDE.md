...
**Immediate next step:** **Commit issue #65 bank-import placement polish** with the MVP hardening final polish, then promote migration `20260607000000_categories_own_only.sql` with the existing `202606040`–`202606060` bundle to staging. Verified 2026-06-01 for issue #65: `svelte-check` 0/0, lint 0 errors (5 existing generated Paraglide warnings), format clean, focused transactions Playwright 12/12, app changed-file secret scan clean. Then resume production group-invite hotfix queue (migrations `20260531000000` + `20260601000000`, prod Vault `max_user_cap`, enable Google signups).
...
**Bank import placement issue #65:** local 2026-06-01 — transactions header no longer uses the three-dot actions menu on desktop; import/export are direct header buttons, and mobile exposes a named `Import bankowy` toolbar control that opens the import/export sheet. Transactions Playwright now covers desktop direct actions, mobile toolbar access, and the date-range mobile test was hardened against current-month drift.
