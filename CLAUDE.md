...
**Bank CSV import V1:** steps 1–5.5 done (see history). **Step 6** — save-as-rule + deterministic categorization rules engine shipped 2026-05-27 (branch `wip`); auto-fills `suggested_category_id`+`selected_category_id` during preview. **Remaining Step 6 backlog:** masked-LLM `suggested_category` seam (rules engine leaves a clean hook; no provider wired). Dashboard AI observability ("Sygnały z okresu") parked 2026-05-24 until core product stable.
