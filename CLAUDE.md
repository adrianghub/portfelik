...
**Immediate next step:** **Commit MVP hardening final polish** (own-only categories, push UI state, settings tabs layout, planning category-first) and promote migration `20260607000000_categories_own_only.sql` with the existing `202606040`–`202606060` bundle to staging. Verified 2026-05-29: `svelte-check` 0/0, lint clean, format clean, `pnpm test:rls` 172/172, local reset through `20260607000000`. Then resume production group-invite hotfix queue (migrations `20260531000000` + `20260601000000`, prod Vault `max_user_cap`, enable Google signups).
