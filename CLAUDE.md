...
**Immediate next step:** **Review/commit shopping-list Planning/Shopping/Done split + Bank Import duplicate-rule migration fix**, then resume the production group-invite hotfix queue. Shopping lists are locally refactored around intent modes: `+page.svelte` detail is now a thin shell over `PlanningView`, `ShoppingView`, and `DoneView`; the index shows fixed Aktywne/Nadchodzące/Zarchiwizowane sections with card variants; `planned_for` and `shopping_started_at` drive deterministic buckets/modes; duplicate archived lists become fresh today/planning copies with unchecked items. Bank Import migration `20260602000002_dedupe_categorization_rules.sql` now fails non-destructively when existing duplicate identities are present instead of deleting user rules. Verified 2026-05-28: Paraglide recompiled; `pnpm exec svelte-check --tsconfig ./tsconfig.json`, `pnpm lint`, `pnpm format:check`, changed-file Prettier check, `pnpm test:unit` 26/26, `supabase db reset`, `pnpm test:rls` 172/172, focused `pnpm exec playwright test e2e/tests/shopping-lists.spec.ts` 1/1, changed-file secret scan clean. Production group-invite hotfix remains queued — root cause verified 2026-05-25: prod Auth has global signups disabled, so invited/new Google accounts hit `signup_disabled`; prod had only one `auth.users` row. Local fix adds pending-invite notification backfill on signup/email change, normalizes invite emails client-side, routes group-invite notifications to `/settings?tab=groups`, and changes `enforce_max_user_cap()` so pending group invitations can create users even when `max_user_cap` is reached. After merging/deploying migrations `20260531000000` + `20260601000000`, set prod Vault `max_user_cap` to the current user count (currently 1) and enable Google signups in Supabase Auth; the DB trigger then keeps non-invited signups blocked while invited emails can join. Bank Import UX pass remains local and covered by mocked E2E: upload → auto-categorize → duplicate badge → commit → redirect → re-upload conflict, save-rule defaults from raw bank text after review-only edits, plus optional rules/categories prefill failure fallback. Most recently shipped (branch `wip`, local commits): **Bank Import hardening** — deterministic categorization-rules engine (`services/categorization-rules.ts` + `import/categorize.ts`) pre-fills `suggested_category_id`+`selected_category_id` on preview insert; per-row "Zapisz regułę" in `ReviewTable`; Settings → "Reguły" tab (`RulesTab.svelte`); "Importuj wszystkie z kategorią" disabled when no eligible rows; uploaded file retained as a removable chip on the upload panel; optional rules/category prefill failures now fall back to manual preview rows; save-rule text now seeds from raw counterparty/description instead of review-only edits. **Duplicate detection is now cross-source** — Path A prior import links, Path B shopping-list expenses, and Path C manual/non-list transactions via migration `20260602000001` (manual window tightened to ±1 day, warnings include matched transaction context). Prior shipped: **command-palette transactions search** (#26, merged); a **P2 follow-up is pending PR into `dev`** on branch `fix/search-query-reset` (commit `c9cd024`). **Known pre-existing red:** `apps/web-svelte/e2e/tests/shopping-lists-stabilization.spec.ts` fails on `main`/base too (8 failures) — unrelated to this work; mocked-fixture issue to triage separately.

**Migration state (verified 2026-05-28):** prod/staging are behind local WIP. Local reset applies through `20260603000000_shopping_lists_planning_modes.sql`; that migration adds shopping-list planning columns, refreshes duplicate-list RPC behavior, and keeps RLS covered by `pnpm test:rls` 172/172. Local also includes hotfix migrations `20260531000000`, `20260601000000`, `20260602000000`, bank-import duplicate warning migration `20260602000001_include_manual_duplicate_warnings.sql`, and non-destructive categorization-rule duplicate guard `20260602000002_dedupe_categorization_rules.sql`, all awaiting normal branch promotion. `SELECT proname FROM pg_proc WHERE prosecdef AND proconfig IS NULL AND pronamespace='public'::regnamespace` returned `[]` on prod before the hotfix (every SECURITY DEFINER fn pins `search_path`). The migration files `20260526000000`–`20260530000000` are future-dated relative to their 2026-05-24/25 authoring but are already applied to prod+staging+local and are **frozen — do not rename** (project rule: never amend applied migrations). Going forward, never apply prod/staging schema out-of-band (MCP `apply_migration` / linked query) — let merges to the synced branch be the only path that mutates remote schema, or history drifts again. The dashboard AI/observability panel ("Sygnały z okresu") remains parked locally until the product is more stable.
