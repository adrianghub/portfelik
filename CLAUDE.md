...
**Immediate next step:** **Bank Import UX pass** (auto-detect/rules/duplicate handling + full Playwright commit/re-upload/duplicate walkthrough). Everything before it is shipped: transactions filter/bulk-ops, shopping-list item categories + editable vocabulary, P2 badge review fixes, and CI hardening (`format:check` + isolated `test:unit` + blocking full-history Gitleaks gating RLS/e2e/deploy) are all **merged to `main`** via PRs #18–#23. Repo clean at `cbffb9d`; `main`, `dev`, `origin/main`, `origin/dev` all in sync.

**Migration state (verified 2026-05-25):** prod == staging == local — all 35 migrations applied, **no drift**; `SELECT proname FROM pg_proc WHERE prosecdef AND proconfig IS NULL AND pronamespace='public'::regnamespace` returns `[]` on prod (every SECURITY DEFINER fn pins `search_path`). The migration files `20260526000000`–`20260530000000` are future-dated relative to their 2026-05-24/25 authoring but are already applied to prod+staging+local and are **frozen — do not rename** (project rule: never amend applied migrations). Going forward, never apply prod/staging schema out-of-band (MCP `apply_migration` / linked query) — let merges to the synced branch be the only path that mutates remote schema, or history drifts again. The dashboard AI/observability panel ("Sygnały z okresu") remains parked locally until the product is more stable.
