...
**Launch certification (2026-06-08):** gates green; manual prod/staging verification done; historical launch baseline closed.
...
**Dashboard trust pass (2026-07-04, on `feat/dashboard-trust-pass`):** dogfooding review fixed six number/logic bugs plus a root-cause boundary bug. (1) **`getDateRangeBounds` TZ fix (all pages):** local-midnight `toISOString()` shifted month ranges by the UTC offset — in Poland July fetched Jun 30 and dropped Jul 31; now date-only strings (`2026-07-01`), `getMonthBounds` deleted (dead). (2) **Dashboard data layer:** five overlapping `fetchTransactions` windows (current/prev/rolling/history/forward) replaced by ONE spanning fetch [min(6-period history, 90-day overdue lookback) … forecast horizon) sliced client-side via `inWindow`; staleTime 60s; year forecast clamped to 1 period. (3) **Upcoming table:** overdue now surfaces from the whole 90-day lookback (previously silently vanished at period rollover), scheduled+projected forward rows included (week view used to drop materialized occurrences), rendered soonest-first via new `TransactionTable.initialSortDirection` prop; "Zobacz wszystko" spans lookback→horizon. (4) **Shared forecast source:** current-period projections (`currentProjectedTxs`) now feed "Z zaplanowanymi", the chart's current bar (paid + scheduled remainder, `isProjected` so it joins the forecast band), and the upcoming list — dashboard now agrees with /transactions for the same window. (5) **Ratio floors:** savings ratio needs income ≥150 zł and clamps to ±100% (was "-1985% przychodów"); ring arc derives from the same page-computed ratio; `CATEGORY_DELTA_FLOOR` 50→150 (was "Mieszkanie ↑898%"). (6) **Comparison windows:** month/year compare period-to-date vs previous period-to-date (Jul 1-4 vs Jun 1-4); week stays rolling; copy "7 dni"/"vs poprzednie 7 dni". (7) **Rename:** Pulpit/Panel → **Kokpit**. (8) **Dead-code purge (−1089 lines):** orphan `/plans/[id]/scenarios` route + Belka/simulation engine + `plan-refinance` service (+specs), shopping-lists redirect stubs, 50 unused pl.json keys, 12 dead service exports; transactions-page scope/view predicates deduped; e2e fixtures use dynamic dates (static May dates were aging out of dashboard windows — `dashboard-mobile` would have broken). Gates: svelte-check 0/0, lint 0, format clean, unit 356/356, components 17/17, secret scan clean, `git diff --check` clean. E2E statically audited (per instruction, not run). Awaiting manual commits + PR to `dev`.

**Bird-eye purge round 2 (2026-07-04, same branch):** dashboard's onboarding-progress persist loop was write-only (checklist UI replaced by guided tour in `924ff56`, nobody read the stored progress) — deleted loop + 3 feeder queries (`import-health`/`financial-snapshot`/`count-probe` off the dashboard) + `onboarding-progress.ts` + spec + never-imported components (`DashboardOnboardingChecklist`, `ProgressRing`, `MonthPicker`); `{#if false}` avatar picker cut from PersonalizationTab (nav still renders stored preset — **avatar is now render-only; decide re-enable vs drop**); pl.json pruned by 265 dead keys total (audit is static-exact; no dynamic `m[...]` access exists); `accentConfettiColors` deleted, `getAccentPreset` privatized. Unit 353/353. Whole pass now nets −1,893 lines.

**Critical hotspot remediation slice 1 (2026-07-13, local on `dev`):** `transactions_with_category` now keeps RLS-visible shared transactions when the viewer cannot read the creator's private category (`security_invoker` + category `left join` + non-sensitive fallback); RLS regressions cover private-row hiding, shared-row visibility, owner category preservation, and direct peer-category denial. Weekly admin summaries now contain counts only, with the production copy/payload builder imported by unit tests; stale amount-bearing notification types were removed and historical `transaction_summary` rows are purged by migration. Gates: `supabase db reset` clean through `20260801010000`, svelte-check 0/0, lint clean, format clean, unit 366/366, components 17/17, RLS 316/316, Edge Function `deno check`, build clean, diff and secret scans clean. Edgar was unavailable (worker API connectivity), so important findings were verified directly.

**Private net-worth semantics (2026-07-14, local on `dev`):** net worth is the signed-in user's private balance sheet. The shared `collectNetWorthDebtBalances` helper now excludes every group-scoped debt, including group plans created by the current user, while group plan cards/progress remain visible elsewhere. Debt-only users now count as having net-worth data, so a real negative balance is not replaced by the empty prompt. Gates: svelte-check 0/0, lint clean, format clean, unit 367/367, components 17/17, build clean, diff check clean.

**Production deploy parity gate (2026-07-14, local on `dev`):** production Pages now depends on a Supabase backend job. It links only the canonical production project, blocks unless every local/remote migration version matches, deploys changed allowlisted Edge Functions (or all managed functions when config changes), and rejects Data API `401` instead of treating a broken publishable key as healthy. The parity parser passes against the reset local history; workflow YAML parses/formats clean. Production environment now requires `PROD_SUPABASE_ACCESS_TOKEN`, `PROD_SUPABASE_DB_PASSWORD`, and `PROD_SUPABASE_PROJECT_REF`.

**Pre-account group invite design (2026-07-14):** `docs/specs/2026-07-14-pre-account-group-invitations-design.md` extends the existing pending-email invitation model with hashed expiring tokens, server-side transactional email, minimal public preview, invite-only Auth redirect preservation, atomic post-auth claim, delivery retry state, abuse controls, and group-dependent financial cache invalidation.

**Low-decision UX contract + local date fix (2026-07-14, local on `dev`):** `intent-oriented-ui.md` now defines default precedence (explicit context → valid confirmed preference → deterministic current/history suggestion → safe product default), storage boundaries, successful-action-only learning, stale-reference validation, and the invariant that financial scope never carries across unrelated workflows. Invite design locks visible group context, defaults member/7-day lifecycle, and never reuses recipient email; save contributions suggest amount from explicit context/unmet pace/same-plan history, cap at remainder, lock visible plan scope, and default to local today. Shared `localDateIso` replaces UTC “today” in transaction creation/status and end-series actions, eliminating Warsaw midnight off-by-one behavior. Gates: svelte-check 0/0, lint/format clean, unit 369/369, components 17/17, build clean, diff check clean.

**Pre-account invitations + save contributions (2026-07-14, local on `dev`):** group owners now send expiring one-time email invitations through `send-group-invitation`; hashes live outside the Data API, token minting/Auth-link generation are service-role-only, public preview is minimal, exact JWT email is atomically claimed, and failed delivery is retryable. Invite-only Auth links work while public signup stays disabled. Membership changes invalidate all cached financial data. Save plans now settle with paid expenses, linked transaction types are DB-locked, `add_plan_contribution` atomically creates a caller-owned Cele expense in the plan scope, legacy synthetic income is converted narrowly, and genuine linked income is only unlinked. The short form uses explicit context → unmet pace → same-plan history, local today, and locked scope. Active private save progress is an automatic net-worth asset; group goals stay excluded. Transaction details support eligible plan link/unlink with debt synchronization. Demo data follows the same model.

Final gates: `supabase db reset` clean, RLS 322/322, unit 371/371,
components 17/17, focused group-invite + plan-settle E2E 9/9,
svelte-check 0 errors/0 warnings, lint and format clean, build clean, both changed
Edge Functions pass `deno check`, and the reviewed secret scan contains no
...
**Immediate next step (2026-07-14):** configure the production Supabase GitHub environment secrets plus the Cloudflare Email Service invitation secrets listed in the invite spec (with a staging recipient allowlist), manually promote migrations `20260801000000` through `20260802000000`, then merge so CI deploys the changed Edge Functions and Pages behind the parity gate. Run staging invite-delivery/claim and contribution/net-worth smoke tests before production.
