-- Group-scoped settlement must require current membership; plan/tx creator alone is not enough.

create or replace function public.can_access_plan_for_settlement(p_plan public.plans)
returns boolean
language sql
stable
...
as $$
  select case
    when p_plan.group_id is null then p_plan.user_id = (select auth.uid())
    else (select public.is_group_member(p_plan.group_id))
  end;
$$;

create or replace function public.can_access_transaction_for_settlement(p_tx public.transactions)
returns boolean
language sql
stable
set search_path = public
as $$
  select case
    when p_tx.group_id is null then p_tx.user_id = (select auth.uid())
    else (select public.is_group_member(p_tx.group_id))
  end;
$$;

revoke all on function public.can_access_plan_for_settlement(public.plans) from public, anon;
revoke all on function public.can_access_transaction_for_settlement(public.transactions) from public, anon;
