-- Issue #75: generic import adapters. Broaden bank_accounts.kind to the full
-- adapter set, add adapter_kind/source_kind to sessions, and switch the commit
-- guard from detected_kind to coalesce(adapter_kind, detected_kind). Short-term
-- path (no table rename); detected_kind retained (legacy). adapter_kind is
-- nullable so existing direct inserts that only set detected_kind keep working.

-- 1. Broaden the bank_accounts.kind check to the full bank adapter set.
alter table bank_accounts drop constraint if exists bank_accounts_kind_check;
alter table bank_accounts
  add constraint bank_accounts_kind_check
  check (kind in (
    'mbank', 'ing', 'pko_bp', 'pekao', 'erste',
    'millennium', 'alior', 'bnp_paribas', 'citi_handlowy'
  ));

-- 2. Add adapter_kind (nullable) + source_kind to sessions.
alter table transaction_import_sessions
  add column if not exists adapter_kind text,
  add column if not exists source_kind  text not null default 'bank_statement';

-- 3. Backfill adapter_kind from the legacy detected_kind for existing rows.
update transaction_import_sessions
   set adapter_kind = detected_kind
 where adapter_kind is null;

-- 4. source_kind value constraint.
alter table transaction_import_sessions
  drop constraint if exists transaction_import_sessions_source_kind_check;
alter table transaction_import_sessions
  add constraint transaction_import_sessions_source_kind_check
  check (source_kind in ('bank_statement', 'portfelik_export'));

-- 5. Commit function: validate coalesce(adapter_kind, detected_kind). Body is
--    otherwise IDENTICAL to 20260608000000_import_inne_fallback.sql.
...
comment on function commit_import_session(uuid) is
  ...
  'Kind guard uses coalesce(adapter_kind, detected_kind) for adapter registry '
  'compatibility (issue #75).';
