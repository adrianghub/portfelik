...
| ------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `internal_trigger_secret` / `INTERNAL_TRIGGER_SECRET` | Supabase Vault + Edge Function secrets | DB triggers/cron → Edge Functions; `send-push`, `send-admin-summary`; fallback for `sync-user-role` | Internal triggers fail until both sides update. Prefer a dedicated `SYNC_USER_ROLE_SECRET` so a push-cron leak cannot elevate admin. Rotate immediately on suspicion. |
| `SYNC_USER_ROLE_SECRET` (optional) | Edge Function secrets (`sync-user-role`) | `sync-user-role` only — elevates `app_metadata.role` | When set, `sync-user-role` ignores `INTERNAL_TRIGGER_SECRET`. Rotate independently of push secrets. |
...
- `supabase/functions/sync-user-role/` - **privileged**: elevates `app_metadata.role` when called with a valid `INTERNAL_TRIGGER_SECRET`. Compromise of that secret is an admin-elevation incident; rotate immediately and audit recent `auth.users` `raw_app_meta_data` changes.
