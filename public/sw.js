...
const importScripts = async () => {
  const { CACHE_NAMES, CACHE_STRATEGIES } = await import('./sw/config/constants.js');
  const { cacheFirst, networkFirst, staticOnly } = await import('./sw/strategies/cacheStrategies.js');
  const { handleNotificationClick, handlePushEvent, handleShowNotification } = await import('./sw/strategies/notificationStrategies.js');
  const { getCacheStrategy } = await import('./sw/utils/requestUtils.js');
